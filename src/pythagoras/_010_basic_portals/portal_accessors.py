...
from .basic_portal_core_classes import _PORTAL_REGISTRY, BasicPortal
from .single_thread_enforcer import _ensure_single_thread


def get_number_of_known_portals(target_class: type = BasicPortal) -> int:
    """Get the number of portals currently in the system.

    Args:
        target_class: Class to filter portals. Default is BasicPortal.
...
def get_all_known_portals(target_class: type = BasicPortal) -> list[BasicPortal]:
...
def get_number_of_active_portals(target_class: type = BasicPortal) -> int:
...
def get_depth_of_active_portal_stack(target_class: type = BasicPortal) -> int:
...
def get_current_portal(target_class: type = BasicPortal) -> BasicPortal:
...
def get_nonactive_portals(target_class: type = BasicPortal) -> list[BasicPortal]:
...
def get_noncurrent_portals(target_class: type = BasicPortal) -> list[BasicPortal]:
