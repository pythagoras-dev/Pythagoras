"""Basic requirement utilities for guarded code portals.

This module contains small, composable requirements used by guarded portals
before executing user functions. Each public factory returns a
SimpleRequirementFn configured with fixed arguments, so requirements can be
attached declaratively to guarded functions.

Execution context:
- When executed by a portal, requirement functions run with two names injected
  into their global namespace: `self` (the ExtensionFn instance) and `pth`
  (the pythagoras package). This allows them to access portal services such as
  system introspection and package management.

Conventions:
- Return NoObjectionsFlag (NO_OBJECTIONS) to indicate the check
  ...
  are treated as FAILURE, not success. Only NO_OBJECTIONS
  (the sentinel singleton) indicates success. The check uses
  identity comparison (``is NO_OBJECTIONS``), not truthiness.
"""

from typing import TYPE_CHECKING

from .._350_guarded_code_portals import SimpleRequirementFn
from .no_objections_const import NoObjectionsFlag
from .._110_supporting_utilities import is_valid_env_name

# The requirement functions below use `pth` and `self` which are injected into
...
def _at_least_X_CPU_cores_free_check(n: int) -> NoObjectionsFlag | None:
    ...
    Returns:
        NO_OBJECTIONS if estimated free cores >= n (within 0.1
...
def unused_cpu(cores: int) -> SimpleRequirementFn:
    """Create a requirement that requires at least the given free CPU cores.

    Args:
        cores: Minimum number of free logical CPU cores required (must be > 0).

    Returns:
        A requirement that succeeds only when the system has at least
    ...
    return SimpleRequirementFn(_at_least_X_CPU_cores_free_check).fix_kwargs(n=cores)


def _at_least_X_G_RAM_free_check(x: int) -> NoObjectionsFlag | None:
    ...
    Returns:
        NO_OBJECTIONS if estimated free RAM >= x (within 0.1
        tolerance); otherwise None.
    """
...
def unused_ram(Gb: int) -> SimpleRequirementFn:
    """Create a requirement that requires at least the given free RAM (GiB).
    ...
    return SimpleRequirementFn(_at_least_X_G_RAM_free_check).fix_kwargs(x=Gb)


def _check_python_package_and_install_if_needed(
        package_name: str) -> NoObjectionsFlag | None:
    ...
    Returns:
        NO_OBJECTIONS if the package is already importable or was
...
def installed_packages(*args) -> list[SimpleRequirementFn]:
    """Create requirements ensuring each named package is available.

    Args:
        *args: One or more package names as strings.

    Returns:
        A list of requirements, one per package name, preserving order.
    ...
    for package_name in args:
        ...
        new_requirement = SimpleRequirementFn(_check_python_package_and_install_if_needed)
        new_requirement = new_requirement.fix_kwargs(package_name=package_name)
        requirements.append(new_requirement)
    return requirements


def _environment_variable_availability_check(name: str) -> NoObjectionsFlag | None:
    ...
    Returns:
        NO_OBJECTIONS if the environment variable is set
...
def required_environment_variables(*names) -> list[SimpleRequirementFn]:
    """Create requirements ensuring each named environment variable is set.

    Args:
        *names: One or more environment variable names as strings.

    Returns:
        A list of requirements, one per environment variable name.
    ...
    Example:
        >>> requirements = required_environment_variables("API_KEY", "DATABASE_URL")
    """
    ...
    requirements = []
    for name in names:
        new_requirement = SimpleRequirementFn(_environment_variable_availability_check)
        new_requirement = new_requirement.fix_kwargs(name=name)
        requirements.append(new_requirement)
    return requirements
